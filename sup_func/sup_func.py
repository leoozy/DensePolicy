...
def execute_with_IS(code_str, global_env={}):
    # global_env = copy.deepcopy(global_env)
    shell = InteractiveShell(
        # NOTE: shallow copy is needed to avoid
        user_ns=global_env,
        colors="NoColor",
    )
    shell.run_cell(code_str, store_history=False)
    del shell
    return global_env
...
def parse_left_args(args, left, argument_func):
    parser_new = argparse.ArgumentParser()
    parser_new = argument_func(parser_new)
    args_data, left_new = parser_new.parse_known_args(left)
    args.__dict__.update(args_data.__dict__)
    return args, left_new


def printc(content, color='reset'):
    color_codes = {
        'black': "\033[30m",
        'red': "\033[31m",
        'green': "\033[32m",
    ...
    if color in color_codes:
        color_prefix = color_codes[color]
    else:
        color_prefix = color_codes['reset']

    print(color_prefix + content + color_codes['reset'])


class PrintAllExpressions(ast.NodeTransformer):
    def visit_Expr(self, node):
        if (isinstance(node.value, ast.Call) and isinstance(node.value.func,
                                                            ast.Attribute) and node.value.func.attr == 'append') or (
                isinstance(node.value, ast.Call) and isinstance(node.value.func,
                                                                ast.Name) and node.value.func.id == 'print'):
            # Skip printing 'append' calls
            return node
        # Wrap all expressions in a print() statement
        print_call = ast.Call(func=ast.Name(id='print', ctx=ast.Load()), args=[node.value], keywords=[])
        print_expr = ast.Expr(value=print_call)
        ast.copy_location(print_expr, node)
        ast.copy_location(print_expr.value, node.value)
        return print_expr



def eval_pd_dataset(dataset_pd):
    def eval_nona(item):
        if pd.isna(item):
            return item
        else:
...
def flat_row(row):
    # specific for codeact_tree format result
    row_list = []
    key_list = []
    for key in row.keys():
    ...
    for key in row.keys():
        if not isinstance(row[key], dict):
            ...
            key_list.append(key)
    res.columns = key_list
    res = res.reset_index().rename(columns={'level_0': 'run_index'})
    return res