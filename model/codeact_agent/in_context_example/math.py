...
math_codeact_prompt_list = [
    ['''
You should perform actions to accomplish the goal: Find the coefficient of $x^3$ when $3(x^2 - x^3+x) +3(x +2x^3- 3x^2 + 3x^5+x^3) -5(1+x-4x^3 - x^2)$ is simplifie.
''',
     [
         '''
Thought: I should first create the sympy expression object for the expression in the question. I can check the object by printing it.
Action:
...
''',
         '''
9*x**5 + 26*x**3 - x**2 + x - 5
'''],
     [
         '''
Thought: I can simplify the expression via simplify from sympy. I can print the coefficient of $x^3$ to get the value of it.
Action:
from sympy import simplify
simplified_expr = simplify(expr)
x3_coefficient = simplified_expr.as_coefficients_dict()[x**3]
print(x3_coefficient)
End Action
''',
         '''
26
'''],
     ['''
Thought: I get the value of coefficient of $x^3$ computed by the Interpreter. Now I can answer the question with the Answer command.
Answer:
26
''',
      '''
